...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (self.taskArr.count < 2) {
        ...
        //这里自己写需要保存数据的路径
        NSString *document = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject;
        NSString *movePath =  [document stringByAppendingPathComponent:@"movie.mp4"];

        
//        BOOL isSuccess = [[NSFileManager defaultManager] copyItemAtPath:_tempPath toPath:movePath error:nil];
        // 资源路径移动，拷贝的话，会占用不必要内存
        BOOL isSuccess = [[NSFileManager defaultManager]moveItemAtPath:_tempPath toPath:movePath error:nil];
        if (isSuccess) {
